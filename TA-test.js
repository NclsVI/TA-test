...
function calculateMenAverageAge(people, century) {
    const _getAverage = arr => arr.reduce((total, item) => total + item, 0) / arr.length
    const ages = people
        .filter(({sex}) => sex == "m")
        .filter(({died}) => century ? Math.ceil(died / 100) == century : true )
        .map(({died, born})=> died - born)

    return _getAverage(ages)
}
...
function calculateWomenAverageAge(people, withChildren) {
    ...
    const ages = people
        .filter(({sex}) => sex == "f")
        .filter(({name}) => withChildren ? people.some(({mother}) => mother == name) : true )
        .map(({died, born})=> died - born)
    return _getAverage(ages)
}
...
function calculateAverageAgeDiff(people, onlyWithSon) {
    ...
    const ageDiff = people
        ...
        .filter(({mother}) => people.some(({name}) => mother == name ))
        .map(({mother, born}) => born - people.find(({name}) => mother == name).born)

    return _getAverage(ageDiff)
}
...
/** PS
 * _getAverage можно вынести в хелпер или в Array.prototype (переиспользуется)
 * не приводил к типам (типу typeof или !!) рассматривал будто у нас идеальные устовия
 */
